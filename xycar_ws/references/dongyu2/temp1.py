...
def start(): 
    ...
    initial_steering_started = False 

    initial_steering_time = 0 

    initial_steering_duration = 2.0  # 2초 동안 오른쪽으로 조향 
    ...
    while not rospy.is_shutdown(): 
        ...
        if ranges is not None:             
            ...
            # 차량이 실제로 움직이기 시작할 때 오른쪽 조향 시작 

            if light_go and speed > 0 and not initial_steering_started: 

                initial_steering_started = True 

                initial_steering_time = time.time() 

                rospy.loginfo("차량 출발 감지: 오른쪽 조향 시작") 

 

            # 초기 오른쪽 조향 처리 

            if initial_steering_started: 

                current_time = time.time() 

                if current_time - initial_steering_time < initial_steering_duration: 

                    steering_angle = 20.0  # 오른쪽으로 20도 조향 

                    rospy.loginfo("초기 오른쪽 조향 중 (조향각: %.1f도)", steering_angle) 
